   ...
   public static void main(String [] arg)
   {
      ...
      if(pass)
      {
         ...
         else
         {
            ...
            {
               System.out.println("Something went wrong. Compare the following with what is in testPlayerFile2.txt");
               System.out.println(fromYourMethod);
            }
         }
      }
   ...
   public static Player readPlayerFromFile(String fileName)
   {
      try
      {
         ...
         if(input.hasNextLine()) {
            ...
            //split the String line around the space character       parts[0]->"-1", parts[1]->"-1", parts[2]->"-1"
            ...
            for (int i = 0; i < parts.length - 1; i += 2) {
               ...
               if (!wordIsInt(parts[i]) || !wordIsInt(parts[i + 1]))
                  return null;
            ...
            //initialize the fileNames ArrayList               (substring),line stores:                           "[images/characters/player/saber1.Gif, images/characters/player/saber2.Gif, images/characters/player/saber3.Gif, images/characters/player/saber4.Gif]"
            //remove '[' and ']'                               (substring),line stores:                           "images/characters/player/saber1.Gif, images/characters/player/saber2.Gif, images/characters/player/saber3.Gif, images/characters/player/saber4.Gif"
            //separate elements around ','                     (split) String [] stores 4 elements:               "images/characters/player/saber1.Gif", "images/characters/player/saber2.Gif", "images/characters/player/saber3.Gif" and "images/characters/player/saber4.Gif"
            //add each trimmed element into fileNames ArrayList(add, trim) fileNames ArrayList stores 4 elements: "images/characters/player/saber1.Gif", "images/characters/player/saber2.Gif", "images/characters/player/saber3.Gif" and "images/characters/player/saber4.Gif"
            ...
            for (int i = 0; i < parts.length; i++) {
               ...
               if (parts[i].trim().length() == 0)
                  continue;
               parts[i] = parts[i].trim();
               Spell spell = Spell.getSpellWithName(parts[i]);
            ...
            parts = line.split(",");
            // loop over parts and print
            for (int i = 0; i < parts.length; i++) {
               // skip over empty strings
               if (parts[i].trim().length() == 0)
                  continue;
               String[] objects = parts[i].trim().split(" ");
               objects[0] = objects[0].replaceAll("\\(", "");
               objects[objects.length - 1] = objects[objects.length - 1].replaceAll("\\)", "");

               Teleporter t = new Teleporter(Integer.parseInt(objects[0].trim()), Integer.parseInt(objects[1].trim()), Integer.parseInt(objects[2].trim()), objects[3].trim());
               memory.add(t);
            }
          ...
          //code to read armorSet and armorFreq from file: Line 17
            ...
            {
               parts[i] = parts[i].trim();
               String[] temporary = parts[i].split(":");
               armorSet.add(Armor.getArmorWithName(temporary[0]));
               armorFreq.add(Integer.parseInt(temporary[1]));
            }
          ...
          //code to read weapons and weaponFrequencies from file: Lines 18-32
            ...
            weapons = new ArrayList[Player.LUTE+1];
            weaponFrequencies = new ArrayList[Player.LUTE+1];

            for(int i=0; i<Player.LUTE+1; i++)
            {
               ...
               for (int j = 0; j < parts.length; j++) {
                  ...
                  String[] temporary = parts[j].split(":");

                  weapons[i].add(Weapon.getWeaponWithName(temporary[0]));
                  weaponFrequencies[i].add(Integer.parseInt(temporary[1]));
               }
            }
            ...
            {
               System.out.println("Error reading weapons and weaponFrequencies from player file");
               return null;
            }
           ...
           //code to read worldCol from file: Line 39
            ...
            Player ans = new Player(name, fileNames, imageIndex, startStoryIndex, mapIndex, row, col, worldRow, worldCol, memory, locationType, mapMark, 
               ...
               weaponSelect, armorSelect, weapons, weaponFrequencies, armorSet, armorFreq, rations, items, effects, potions, flowerBoxCount, spells, numArrows, dogBasicInfo, horseBasicInfo,
   ...
   public static LinkedList<Mission> readMissionStack(String fileName)
   {
      try{
         ...
         //make new mission, add it to the missionStack
            mission = new Mission(type, story, worldRow, worldCol, gold, item, clearLoc, target, targetHolder, targetRow, targetCol, failed);
   ...
   public static boolean writeOutAllNPCFile(ArrayList<ArrayList<NPCPlayer>> collection, String fileName)
   {
      ...
      try{
         ...
         if(CultimaPanel.ENCODE_FILES)
            ...
            writer.println((Player.rollDie(1,1000)*2)+1);
         writer.println(encode("**name,charIndex,mapIndex,row,col,homeRow,homeCol,missionIndex,might,mind,agility,body,reputation,hasMet,restoreDay,endChaseDay,origMoveType,locationType,numInfo,weapon,armor,items,gold,effect", CultimaPanel.ENCODE_FILES));
...
//writes out all egg harvest to restore information into a file
   ...
   public static ArrayList<RestoreItem> readHarvestToRestore(String fileName)
   {  //egg harvest to restore in the form of (mapIndex row col value)
      try
      { 
         Scanner input = new Scanner(new FileReader(fileName));
         ArrayList<RestoreItem> ans = new ArrayList<RestoreItem>();
